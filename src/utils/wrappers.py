...
from utils.geometry import undistort_points
...
class TensorWrapper:
    ...
    @classmethod
    def reshape(cls, obj, shape):

        last_dim = obj._data.shape[-1]
        new_shape = [x for x in shape]
        new_shape.append(last_dim)

        data = torch.reshape(obj._data, new_shape)
    ...
    def __torch_function__(cls, func, types, args=(), kwargs=None):
        ...
        if func is torch.stack:
            ...
            return cls.reshape(*args, **kwargs)
        else:
