...
def main():
    ...
    width_ratios = [1] * cols
    width_ratios.append(0.38)
    col_size = sum([i*8 for i in width_ratios])


    # Plot!!!!!
    fig, axes = plt.subplots(rows, len(width_ratios), sharex=False, sharey=True, figsize=(col_size, 4*rows), squeeze=False, gridspec_kw={"width_ratios":width_ratios})
    ...
    last_top = 1.05
    for legend_name in legend_configs.keys():

        specific_legend_configs = legend_configs[legend_name]

        # Get the handles and labels for the line
        handles = list()
        labels = list()
        for legend_item_idx in specific_legend_configs["lines"].keys():

            # Get the values for this legend item
            legend_item_cfg = specific_legend_configs["lines"][legend_item_idx]
            ...
            # Line style
            line_style = legend_item_cfg["line_style"]
            line_style = linestyle_mapping[line_style]
            line_style = linestyle_tuple[line_style]

            # Get the color
            color = sns.color_palette("tab10")[color_index]
            l = mLines.Line2D([0, 10], [0, 1], color=color, linewidth=2.5, alpha=color_alpha, linestyle=line_style)
        ...
        # get the specific configs
        title = specific_legend_configs["title"]
        title_fontsize = specific_legend_configs["title_fontsize"]

        # Get the legend sizing information
        legend_tmp = plt.legend(handles, labels,loc=(0, 0), ncols=1, fontsize=10, title=title, fancybox=True, title_fontsize=title_fontsize, handlelength=3)
        bbox = legend_tmp.get_tightbbox().transformed(axes[0, -1].transAxes.inverted())
        height = bbox.y1

        # Compute the new legend loc
        y = last_top - height
        location = (0, y)

        # Add the legend
        legend1 = plt.legend(handles, labels, loc=location, ncols=1, fontsize=10, title=title, fancybox=True, title_fontsize=title_fontsize, handlelength=3)
        legend1._legend_box.align = "left"
        leg = axes[0, -1].add_artist(legend1)
        ...
        # Get the bbox
        bbox = leg.get_tightbbox().transformed(axes[0, -1].transAxes.inverted())

        # Set the last top
        padding = 0.025
        last_top = y - padding

    # Turn off the axis for the legend axis
    axes[0, -1].axis("off")
